Shader "Fzzy/Spikes" {
	...
	SubShader {
		...
		#pragma surface surf Stepped fullforwardshadows vertex:vert alpha:fade
		#pragma target 3.0
		...
		void vert (inout appdata_full v, out Input o) {
			...
			float value = tex2Dlod(_DisplacementTexture, v.texcoord*sin(_Time / 50) * 7).x * 5;
			v.vertex.xyz += v.normal * sin(v.vertex.x * 2 + _Time.w) * 0.1;

			UNITY_INITIALIZE_OUTPUT(Input, o);
			o.displacementValue = value / 5;
		}
		...
		void surf (Input i, inout ToonSurfaceOutput o) {
			...
			//col.rgb = lerp(col.rgb * col.a, float3 (0, 0, 0), i.displacementValue);
			col.g = 0.1;
			col.b = 0.1;
			col.r = i.displacementValue;
